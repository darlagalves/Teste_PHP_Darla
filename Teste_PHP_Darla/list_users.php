...
<body>
    ...
    <div class="d-flex justify-content-between mb-4">
        <button onclick="window.location.href='register.php'" class="btn botaoadd">Adicionar Usuário</button>
        <form class="form-inline" method="GET" action="">
            <input class="form-control mr-2" type="search" placeholder="Pesquisar por nome ou CPF" aria-label="Pesquisar" name="search" style="width: 300px;" required>
    ...
    <table border="1" class="tabela">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="edit_user.php" method="post" onsubmit="return validarFormulario(<?php echo $row['id']; ?>)">
                        <input type="hidden" name="id" value="<?php echo $row['id']; ?>">
                        <div class="form-group">
                            <label for="nome">Nome:</label>
                            <input type="text" class="form-control" id="nome<?php echo $row['id']; ?>" name="nome" value="<?php echo $row['nome']; ?>" required>
                        </div>
                        <div class="form-group">
                            <label for="cpf">CPF:</label>
                            <input type="text" class="form-control" id="cpf<?php echo $row['id']; ?>" name="cpf" value="<?php echo $row['cpf']; ?>" required>
                        </div>
                        <div class="form-group">
                            <label for="email">Email:</label>
                            <input type="email" class="form-control" id="email<?php echo $row['id']; ?>" name="email" value="<?php echo $row['email']; ?>" required>
                        </div>
                        <div class="form-group">
        ...
        <div>
            ...
            <?php for ($i = 1; $i <= $total_paginas; $i++): ?>
                <?php if ($i == $pagina_atual): ?>
                    <span class="btn btn-primary disabled"><?php echo $i; ?></span>
                <?php else: ?>
                    <a href="?pagina=<?php echo $i; ?>" class="btn btn-outline-primary"><?php echo $i; ?></a>
                <?php endif; ?>
            <?php endfor; ?>

            <?php if ($pagina_atual < $total_paginas): ?>
                <a href="?pagina=<?php echo $pagina_atual + 1; ?>" class="btn btn-secondary">Próxima</a>
            <?php endif; ?>
        </div>
        <div>
            <span><?php echo "Página $pagina_atual de $total_paginas"; ?></span>
        </div>
    </div>
